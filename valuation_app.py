...
all_prices_standardize = []
for i in all_prices:
    all_prices_standardize.append(all_prices[-1] - i)
...
line_y = [现货价, 次主力合约_price, 次主力合约_price +  ((len(labels)-1)/次主力合约_index) * abs(现货价 - 次主力合约_price)]  # 延长线保持相同斜率
